	...
	$.cxpSearchform = function(el, options) {
		...
		var autoSuggestUpdate = function($elem,json,val) {
			...
			if(json) {
				$.each(json, function(key,values) {
					if(key && values.length>0) {
						contents += '<div class="tt-dataset"><span class="typeahead-section-name">'+key+'</span><span class="tt-suggestions">';
			...
			if($elem.next('.tt-dropdown-menu').length<1) {
				$elem.after('<div class="tt-dropdown-menu"><div class="tt-text-intro">'+$elem.data('suggestion-text')+'</div><div class="tt-dropdown-contents">'+contents+'</div></div>');
			} else {
